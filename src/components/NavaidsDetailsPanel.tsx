...
interface NavaidsDetailsPanelProps {
    navaids: FeatureData[]
    isOpen: boolean
...
export const NavaidsDetailsPanel = ({
    navaids,
    isOpen,
    onToggle,
    addFavourite,
    removeFavourite,
    isFavourite,
}: NavaidsDetailsPanelProps) => {
    // Don't render if there are no navaids
    if (navaids.length === 0) return null

    return (
        <CollapsiblePanel title="Navaid" isOpen={isOpen} onToggle={onToggle}>
            <div
                style={{
            ...
            >
                {navaids.map((feature, index) => {
                    ...
                    // Extract frequency from name_label_full (e.g., "322.000 kHz" or "117.900 MHz")
                    let frequency = ''
                    if (props.name_label_full) {
                        const match = String(props.name_label_full).match(/(\d+\.\d+)\s*(kHz|MHz)/i)
                        if (match) {
                            frequency = `${match[1]} ${match[2]}`
                        }
                    }
